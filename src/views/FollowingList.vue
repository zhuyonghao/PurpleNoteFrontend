            ...
            <div class="flex items-center space-x-3 flex-1 min-w-0">
              ...
              <div class="flex-1 min-w-0">
                ...
                <div class="flex items-center space-x-4 text-xs text-gray-400 mt-1">
                  ...
                  <span v-if="user.id !== userStore.userInfo?.id && user.mutualFollowsCount > 0" class="text-purple-500">
...
const loadMutualFollowsCountAsync = async (users, startIndex = 0) => {
  ...
  for (let i = 0; i < users.length; i++) {
    ...
    try {
      ...
      console.log(`与用户${user.id}的共同关注人数:`, mutualCount)
      ...
      // 设置默认值
      if (followingList.value[userIndex]) {
        followingList.value[userIndex].mutualFollowsCount = 0
      }
    }
  }
}
  ...
  try {
    ...
    if (response && response.list) {
      const users = response.list.map(user => ({
        ...
        mutualFollowsCount: 0 // 初始化共同关注人数
      }))
      ...
      // 先加载关注状态，再加载粉丝数，最后加载共同关注人数（顺序执行）
